...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // GET /cat.php?name="some" HTTP/1.1
            char* query;
            if (questionmark != -1)
            {
                query = malloc(sizeof(char)*(space2 - questionmark - 1));
                memcpy(query,line + questionmark + 1,space2 - questionmark - 1);
                query[space2 - questionmark - 1] = '\0';
            }
            else
            {
            ...
            if (questionmark != -1)
            {
                path = malloc(strlen(root) + questionmark - space1);
                memcpy(path,root,strlen(root));
                memcpy(path + strlen(root),line + space1 + 1,questionmark - space1 - 1);
                path[strlen(root) + questionmark - space1 - 1] = '\0';
            }
            else
            {
                path = malloc(strlen(root) + 1 + space2 - space1);
                memcpy(path,root,strlen(root));
                memcpy(path + strlen(root) ,line + space1 + 1,space2 - space1 - 1);
                path[strlen(root) + space2 - space1 - 1] = '\0';   
            }
            ...
            // .php?n=bd 
            // 0123456789
            int exlength = (questionmark != -1)?(questionmark - dot):(space2 -dot);
            char extension[exlength];
            memcpy(extension,line + dot + 1,exlength);
            extension[exlength - 1] = '\0';
